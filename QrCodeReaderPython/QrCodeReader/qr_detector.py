...
def extract_matrix(image, output_size=None):
	...
	Keyword arguments:
	image -- The index in hierarchy to check
	...
	def get_dept(index):
		...
		'''
		# the child is stored in hierarchy at inner index = 2, it is negative
		# if
		# it has no child
   ...
   # checking if size is enough for getting good values
	...
	# take the pattern triple of the one with the greatest distance between
	# BottomLeft and TopRight
	_, patternindex_triple = max(distance_patternlist_tuple_list) 

	# Reordering the and selecting the required contours and centers
	...
	def pattern_corner_generator():
		...
		for contour, center in zip(pattern_contour_list , pattern_center_list):
			# creating triples of:
			#   an tuple of booleans indicating if they are up or down, left or
			...
			# remove the boolean tuple and the distance and store only the
			# corner
	...
	if version > 1: # check if the QR code has a version greater 1.  Then it has a alignment
		...
		# searching this center should be big enough to handle this offset
