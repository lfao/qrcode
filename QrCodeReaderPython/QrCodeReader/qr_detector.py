...
def extract_matrix(image, output_size=None):
    ...
    def get_dept(index):
        ...
        '''
        # the child is stored in hierachy at inner index = 2, it is negative if
        # it has no child
   ...
   # checking if size is enough for getting good values
    ...
    # take the pattern tripple of the one with the greatest distance between
    ...
    def pattern_corner_generator():
        ...
        for contour, center in zip(pattern_contour_list , pattern_center_list):
            # creating triples of:
            #   an tuple of bools indicating if they are up or down, left or
            #   right.  Sorting these ascending will cause the order TL, TR,
            #   BL, BR
            #       therefore the sign of the crossproduct of two vectors is
            #       used:
            ...
            # using the bool tuple
            # (false, false) <=> TL vs.  (false, true) <=> TR vs.  (true,
            # false) <=> BL vs (true, true) <=> BR
            ...
            # for each corner.
            # The key of each categorie is not required since the order is
            ...
            # remove the bool tuple and the distance and store only the corner
    ...
    estimated_pixel_count = size_average / pattern_average * 7 # the width and the heigth of Finder Pattern is 7.  Use the rule of three
    version = int(round((estimated_pixel_count - 17) / 4)) # only pixelcounts of 4 * Version + 17 are allowed => round to this number
    if not 0 < version <= 40:
        raise QrDetectorError('Calculated version {} is not beween 1 and 40!'.format(version))
    ...
    if version > 1: # check if the QR code has a version greater 1.  Then it has a alignment
        ...
        alignment_area_delta = pixel_lenght * 8 # The width and height of the area for searching the contour of the
                                                # finderpattern will be 2 * aligment_area_delta
        ...
        # this coordinates are a half pixel away from the center.  the area of
        # searching this center should be big enought to handle this offset
        ...
        # filter this with only checking if one contour point is inside a
    ...
    if output_size:
        ...
        if version > 1: 
            ...
            # (numpy style adding a 2 element column vector to a 2 elment row
