...
def extract_int_list(numpy_array, start_bit, word_length, word_count):
	'''
	Extract a numpy array of integers out of a numpy array of boolean by interpreting the boolean as bit.
	Keyword arguments:
...
def extract_int(numpy_array, start_bit, word_length):
	'''
	Extract a integer out of a numpy array of boolean by interpreting the boolean as bit.
	Keyword arguments:
	numpy_array -- the numpy array of boolean
	start_bit -- the index of the first bit to be extracted
...
def get_dataarea_indicator(version):
	'''
	Generate a numpy matrix of booleans which values indicate, whether the field of an qr code with the same index contains data or function pattern
	Keyword arguments:
	...
	'''
	size = version * 4 + 17
	retval = numpy.ones((size,size), dtype=bool)    # generate a matrix of true booleans which are indicating data area
	...
	if version > 1 : # remove alignment pattern for every version containing these pattern
		...
		# furthermore the distances to the remaining alignment patterns are
		...
		# the alignment pattern always have the same gaps so they are created
...
def extract_bit_array(bit_matrix, mask_index, output=False):
	...
	bit_matrix = numpy.logical_xor(bit_matrix, mask_matrix, bit_matrix) # invert the pixels of the original image, which are indicated by the mask
	
	# create lists for going up and down a data row consisting of two pixel
	# rows
	...
	# the amount of tuples only depend on the range because itertools.cycle
	# repeats its elements infinite
	...
	# the tuples would contain all fields, even it is no data field but for
	# example finder pattern, alignment pattern or other
	# this have to be filtered out.  Therefor the dataarea_indicator is used.
	# It is true if the matrix contain at the specified coordinates data
...
def error_correction(raw_bit_array, version, ecc_level):
	...
	Returns:
	A list of booleans, which have been corrected with the error correction algorithm (This will contain less elements than raw_bit_array)
	'''
	...
	# distance and start after the all data blocks
	...
	# concatenate the bytes of each block to one array
...
def extract_string(corrected_bit_array, version):
	...
	try:
		while mode_value in SUPPORTED_MODES: #TODO: testing every mode, mixed modes
			...
			# and the amount of chars being in the incomplete last word.  zero
			...
			# Maybe there are some character left which are coded i less than
			# word_length bits
			...
			if mode_index == 0: # numeric mode - words of 10 bits contain 3 chars (0 - 9)
				...
				if remaining_chars_count > 0:
					# extracting 1 or 2 remaining chars in the incomplete word
					# of 4 or 8 bits
