...
def get_dataarea_indicator(version):
    ...
    if version > 1 : # remove alingnment pattern for every version containing these pattern
        ...
        # alignment patttern because of the finder pattern
...
def extract_bit_array(bit_matrix, mask_index, output=False):
    ...
    # rows
    # this contains tulples of the horizontal offset 0 or 1 (column offset) and
    ...
    # the amount of tuples only depend on the xrange because itertools.cycle
    # repeats its elements infinite
    # later the offset must be substracted from the horizontal position to get
    ...
    # the tuples would contain all fields, evan it is no data field but for
    # example finder pattern, alignment pattern or other
    # this have to be filtered out.  Therefor the dataarea_indicator is used.
    # It is true if the matix contain at the specified coordinates data
...
def error_correction(raw_bit_array, version, ecc_level):
    ...
    codeword_count = CODEWORD_COUNT_LOOKUP[version - 1][ecc_level] # the sum of data bytes without error correction overhead being containend in
                                                                   # all blocks
    ...
    #long_codeword_block_bytes_count = short_block_bytes_count + 1 # not
                                                                        #required.  The amount of bytes in long blocks is 1 greater than the short
                                                                        #blocks
    ...
    # The indices of the short blocks are completly regular and have always the
    ...
    # The errorcorrection blocks are all of the same size, have a regular
...
def extract_string(corrected_bit_array, version):
    ...
    try:
        while mode_value in SUPPORTED_MODES: #TODO: testing every mode, mixed modes
            ...
            if mode_index == 0: # numeric mode - words of 10 bits contain 3 chars (0 - 9)
                ...
                # extracting words of 1 chars.  Since there is only 1 char per
                                                 # word, no incomplete words exist
        ...
        raise_from("QR data inconsistent!", e)
