    ...
    for i, filename in enumerate(filepath_list, 1):
        ...
        if image is False:
            ...
            try:
                ...
                cv2.imshow("dataarea", cv2.resize(dataarea_indicator.astype(float), (size * 8, size * 8), interpolation = cv2.INTER_NEAREST))
                ...
                print('Error while detecting occured: {}'.format(e), file = sys.stderr)
            except ReedSolomonError as e:
                print('Error while applying errorcorrection occured: {}'.format(e), file = sys.stderr)
            except QrDecoderError as e:
                print('Error while decoding occured: {}'.format(e), file = sys.stderr)
            else:
                success_counter += 1
                print(string)
            finally:
                print("{} of {} succeeded. Successrate: {}%".format(success_counter, i, round(float(success_counter) / i * 100, 2)))  
