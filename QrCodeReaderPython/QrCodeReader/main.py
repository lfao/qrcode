    ...
    for i, filename in enumerate(filepath_list, 1):
        ...
        if image is False:
            ...
            try:
                ...
                cv2.imshow('9 mask', cv2.resize(numpy.logical_not(mask_matrix).astype(float), (size * 8, size * 8), interpolation = cv2.INTER_NEAREST))
                cv2.imshow('10 data area', cv2.resize(dataarea_indicator.astype(float), (size * 8, size * 8), interpolation = cv2.INTER_NEAREST))
                cv2.imshow('11 data', cv2.resize(numpy.logical_not(bit_matrix_unmasked).astype(float), (size * 8, size * 8), interpolation = cv2.INTER_NEAREST))
